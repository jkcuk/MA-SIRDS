...
export class RDASRenderer {
  ...
  public maxRecursionDepth: number = 20; // Maximum recursion depth for bairns
  public maxDots: number = 200000; // Maximum number of dots
  public dots: number = 0;
  public maxClans: number = 20000; // Maximum number of clans (i.e. parent dots) to be drawn
  public blobSigma: number = 1; // Gaussian width of Gaussian-shaped dots ("blobs")
  public fadeFactor: number = 0.9;
  public minBrightness: number = 0.2;
  public alreadyThereThreshold: number = 250; // threshold for determining if a dot is already there (in the range [0, 1])
  ...
  constructor(
    ...
    this.screenRectangle = new Rectangle(
      ...
      screen.hHalfAxis,
      screen.vHalfAxis,
      screen.hHalfAxis.length() * 2,
      screen.vHalfAxis.length() * 2,
      new Colour([1,0,0]) // material is not needed for the screen rectangle
    );
  }

  addFamilyDots(
    pParentDot: Vector3, // position of parent dot
    rgbComponentIndex: number,  // red = 0, green = 1, blue = 2
    brightness: number,
    iIOD: number, // index of interocular-axis direction
    iCamera: number,  // index of camera that corresponds to interocular-axis direction #iIOD; 0 or 1
    scene: Scene[], // array of scenes; need at least one per interocular-axis direction
    recursionDepth: number // current recursion depth
  ) {
    if(recursionDepth > this.maxRecursionDepth || this.dots >= this.maxDots || brightness < this.minBrightness) return; // Stop recursion if maximum level reached or if maximum number of dots reached
  ...
  addBairnDot(
    ...
    if(this.dots >= this.maxDots) return; // Stop recursion if maximum number of dots reached
    ...
    const ray1 = new Ray(
      this.cameraPosition[iIOD][iCamera], 
      pParentDot.sub(this.cameraPosition[iIOD][iCamera])
    );

    // ... find where it intersects scene #iod, ...
    const sceneHit1 = scene[iIOD].hit(ray1);

    if(!sceneHit1) return; // No intersection with scene
    ...
    if (!screenHit) return; // No intersection with screen plane
    ...
    if(sceneHit2 && sceneHit2.t < sceneHit12camera2.length()) return; // There is an intersection with the scene before reaching the eye

    // check if there is already a blob there
    if(
      this.screen.getRGBComponent(screenHit.h, screenHit.v, rgbComponentIndex) > this.alreadyThereThreshold  // 255*this.minBrightness
    ) {
